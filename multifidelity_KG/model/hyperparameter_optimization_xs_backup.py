...
def covariance(hyperparameters_without_noise, lengths_sq_0, point_one, point_two):
    ...
    # poly_s = ( (point_two[0]-point_one[0])**2 + 1 ) / lengths_sq_0[0]
    # result = hyperparameters_without_noise[0] * exp_value * poly_s
...
def compute_covariance_matrix(hyperparameters_without_noise, noise_hyperparameters, points_sampled):
    '''
...
def compute_delta_observations(points_sampled, points_sampled_value):
    if (len(points_sampled) != len(points_sampled_value)):
        return None
...
def hyper_opt(points_sampled, points_sampled_value, init_hyper, hyper_bounds, approx_grad, hyper_prior=None):
    '''
    ...
    def obj_func(x):
        ...
        covariance_matrix = compute_covariance_matrix(hyperparameters_without_noise, noise_hyperparameters, points_sampled)
        print(covariance_matrix)
        print( scipy.linalg.eigvalsh(covariance_matrix) )
...
def generate_hyperbounds(problem_search_domain, upper_bound_noise_variances, upper_bound_signal_variances=1000.):
    ...
    for (lb,ub) in problem_search_domain:
...
def optimize_hyperparameters(problem_search_domain, points_sampled, points_sampled_value,
    ...
    # num_jobs
    parallel_results = Parallel(n_jobs=1)(delayed(hyper_opt)(points_sampled, points_sampled_value, 
