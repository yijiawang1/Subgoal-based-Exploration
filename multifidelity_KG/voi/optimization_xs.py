...
from multifidelity_KG.voi.knowledge_gradient_xs import *
# import sql_util
...
def negative_kg_given_x_prime(s, all_S_x_prime, noise_and_cost_func, gp, hyperparameters_noise):
    def negative_kg(x):
        s_x = np.hstack((s*np.ones(1), x))
        return -compute_kg_given_x_prime(s, x, all_S_x_prime, hyperparameters_noise, noise_and_cost_func(s_x)[1], gp)
