...
def get_discounted_returns(episodes):
    returns = []
    for episode in episodes:
        rewards = episode.rewards.numpy()
        returns_episode = []
        for sample in range(rewards.shape[1]):
            _return = 0
            for step in range(rewards.shape[0]):
                _return = episode.gamma * _return + rewards[step, sample]
            returns_episode.append(_return)
        returns.append(returns_episode)
    return np.array(returns)

def get_costs(episodes):
    ans = [episode.lengths for episode in episodes]
    return np.array(ans)
