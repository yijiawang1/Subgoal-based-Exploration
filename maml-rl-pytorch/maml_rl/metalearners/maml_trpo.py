...
class MAMLTRPO(GradientBasedMetaLearner):
    ...
    async def _adapt(self, train_futures, first_order=None):
    ...
    def _hessian_vector_product(self, kl, damping=1e-2):
    ...
    async def _surrogate_loss(self, train_futures, valid_futures, old_pi=None):
        first_order = (old_pi is not None) or self.first_order
        params = await self._adapt(train_futures, first_order=first_order)
    ...
    def step(self,
        ...
        old_losses, old_kls, old_pis = self._async_gather(
            [self._surrogate_loss(train, valid, old_pi=None)
        ...
        _hessian_vector_product = self._hessian_vector_product(old_kl, damping=cg_damping)
        stepdir = conjugate_gradient(_hessian_vector_product, grads, cg_iters=cg_iters)

        # Compute the Lagrange multiplier
        shs = 0.5 * torch.dot(stepdir, _hessian_vector_product(stepdir, retain_graph=False))
        ...
        for _ in range(ls_max_steps):
            ...
            losses, kls, _ = self._async_gather([
                self._surrogate_loss(train, valid, old_pi=old_pi)
